...
select uy.as_of_date, 
       case when ag.channel <> '10'
       then case when ag.location_cd = '01' then ag.bu
                 else 'BV'
             end         
       else 'TOC'
       end bu,
       ...
       sum((p.prem_full_term*uy.pct_share)/100) prem_full_term , 
       uy.pct_share
